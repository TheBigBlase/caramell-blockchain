...
contract Data {
	...
	function getAllData(Client storage client, uint8 i) internal view returns(string memory){
		//get all data from a client, and format in json like fashion
		string memory res = "";
		for(uint k = 0 ; k < client.keyList.length ; k++) {
			bytes32 idx = client.keyList[k];
			res = string.concat(res, indent(i - 1), "{\n");
			res = string.concat(res, indent(i));
			res = string.concat(res, '"dataName":', client.data[idx].name, ',');
			res = string.concat(res, indent(i));
			res = string.concat(res, '"dataValue":', uint2str(client.data[idx].data), '\n');
			res = string.concat(res, indent(i - 1), "},\n");
		}
		return res;
	}
